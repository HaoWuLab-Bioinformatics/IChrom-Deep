...
for index_train, index_test in gkf.split(label, groups=chr_name):
    ...
    # 完整测试集评估
    auprc[i] = average_precision_score(label_test, model_pro)
    bacc[i] = balanced_accuracy_score(label_test, model_class)
    mcc[i] = matthews_corrcoef(label_test, model_class)
    precision[i], recall[i], f1[i], _ = precision_recall_fscore_support(label_test, model_class, average='binary')
    print('auprc:', auprc)
    print('bacc:', bacc)
    print('mcc:', mcc)
    ...
    # 平衡测试集评估
    auroc_bal[i] = roc_auc_score(label_test_bal, model_pro_bal)
    ...
    precision_bal[i], recall_bal[i], f1_bal[i], _ = precision_recall_fscore_support(label_test_bal, model_class_bal,
                                                                                    average='binary')
    print('auroc_bal:', auroc_bal)
    print('acc_bal:', acc_bal)
    print('mcc_bal:', mcc_bal)
    print('precision_bal:', precision_bal)
    print('recall_bal', recall_bal)
    print('f1_bal:', f1_bal)
    print(auprc[i], bacc[i], mcc[i], precision[i], recall[i], f1[i])
    print(auroc_bal[i], acc_bal[i], mcc_bal[i], precision_bal[i], recall_bal[i], f1_bal[i])
    print(len(index_test))

    i += 1

print('10-fold cross-validation')
print('auprc:', auprc.mean())
print('bacc:', bacc.mean())
print('mcc:', mcc.mean())
print('precision:', precision.mean())
print('recall', recall.mean())
print('fscore:', f1.mean())
print('auroc_bal:', auroc_bal.mean())
...
print(auprc.mean(), bacc.mean(), mcc.mean(), precision.mean(), recall.mean(), f1.mean())
print(auroc_bal.mean(), acc_bal.mean(), mcc_bal.mean(), precision_bal.mean(), recall_bal.mean(), f1_bal.mean())
print('----------------------------------------------------------------')
for i in range(10):
    print(auprc[i], bacc[i], mcc[i], precision[i], recall[i], f1[i])
print('----------------------------------------------------------------')
